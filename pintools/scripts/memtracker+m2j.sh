...
env LD_LIBRARY_PATH=../wt-dev/build_posix/.libs:../wt-dev/build_posix/ext/compressors/snappy/.libs/  pin.sh   -t $CUSTOM_PINTOOLS_HOME/obj-intel64/memtracker.so -- ./db_bench_wiredtiger --cache_size=534217728  --use_existing_db=1 --threads=8 --use_lsm=1 --db=/tmpfs/leveldb --reads=1000 --benchmarks=readseq --value_size=62 | tee readseq-with-stats.txt | $MEMTRACKER_HOME/memtracker2json.py | $MEMVIS_HOME/analyzer -p "LevelDB readseq on WT with stats" -d $MEMVIS_HOME/server/data
