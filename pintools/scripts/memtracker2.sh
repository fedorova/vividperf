...
# Create the DB
env LD_LIBRARY_PATH=../wt-dev/build_posix/.libs:../wt-dev/build_posix/ext/compressors/snappy/.libs/ ./db_bench_wiredtiger --cache_size=534217728 --threads=1 --use_lsm=1 --db=/tmpfs/leveldb2 --benchmarks=fillseq --value_size=62 

env LD_LIBRARY_PATH=../wt-dev/build_posix/.libs:../wt-dev/build_posix/ext/compressors/snappy/.libs/  pin.sh   -t $CUSTOM_PINTOOLS_HOME/obj-intel64/memtracker.so -- ./db_bench_wiredtiger --cache_size=534217728  --use_existing_db=1 --threads=8 --use_lsm=1 --db=/tmpfs/leveldb2 --reads=1000 --benchmarks=readseq --value_size=62 > log8-lsm-readseq-stats2.txt
