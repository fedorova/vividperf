...
env LD_LIBRARY_PATH=../wt-dev/build_posix/.libs:../wt-dev/build_posix/ext/compressors/snappy/.libs/  time  pin.sh -separate_memory 1 -t $CUSTOM_PINTOOLS_HOME/obj-intel64/memtracker.so -- ./db_bench_wiredtiger --cache_size=534217728  --use_existing_db=1 --threads=2 --use_lsm=0 --db=/tmpfs/leveldb --reads=1000 --benchmarks=readrandom --value_size=62 > memtracker-log4.txt
